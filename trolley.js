// trolley.js
  ...
  // main exports 
    ...
    export function c(p,...v) {
      return trolley(p,v,{useCache:false});
    }

  // main function (TODO: should we refactor?)
    function trolley(p,v,{useCache:useCache=true}={}) {
      const retVal = {};
  ...
  // helpers
    ...
    function summonPlaceholder(sibling) {
      let ph = [...sibling.parentNode.childNodes].find(
        node => node.nodeType == Node.COMMENT_NODE && node.nodeValue == 'trolley-placeholder' );
      if ( ! ph ) {
        ph = toDOM(`<!--trolley-placeholder-->`).firstChild;
      }
      return ph;
    }
    ...
    // Markup helpers
      // Returns an object that Trolley treats as markup,
      // even tho it is NOT a Trolley Object (defined with R/X/$)
      ...
      // Returns an object that Trolley treats, again, as markup
    ...
    // other helpers
      ...
      function replaceValWithKeyAndOmitInstanceKey(vmap) {
        return (val,vi) => {
          ...
          let k = key;
          if ( T.check(T`TrolleyObject`, val) || T.check(T`MarkupObject`, val) ) {
            k = `<!--${k}-->`;
          }
      ...
      function guardAndTransformVal(v) {
        ...
        const isTrolleyArray   = T.check(T`TrolleyArray`, v);
        ...
        const isTrolley        = T.check(T`TrolleyObject`, v);
        const isForgery       = T.check(T`TrolleyLikeObject`, v)  && !isTrolley; 

        if ( isFunc )             return v;
        if ( isTrolley )           return v;
        if ( isKey(v) )           return v;
        if ( isHandlers(v) )      return v;
        if ( isTrolleyArray )      return join(v); 
      ...
      function didChange(oldVal, newVal) {
        ...
        if ( oldType != newType ) {
          ...
          switch(oldType) {
            case "trolleyobject":
              // the trolley object is returned by a view function
